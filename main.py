"""Orchestrates the pipeline: ingest -> screen -> deliver screening result ->
draft -> deliver draft.

All stages are wired to real logic. Ingest/deliver go through
...
from src.deliver import run_delivery, run_screening_delivery
...
def main() -> None:
    ...
    can_deliver = bool(chat_id) and not chat_id.startswith("REPLACE_ME")
    ...
    for note_id, result in screened:
        ...
        print(f"    {result['bucket_explanation']}")

    if can_deliver:
        screening_delivered = run_screening_delivery(client, chat_id)
        print(f"\nDelivered {len(screening_delivered)} screening result(s) via Telegram: {screening_delivered}")
    else:
        print("\nTELEGRAM_CHAT_ID not set — skipping screening-result delivery.")
    ...
    if can_deliver:
        ...
        print("TELEGRAM_CHAT_ID not set — skipping draft delivery.\n")
